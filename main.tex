...
% Numbering scheme per request:
% Section 1 -> I; Sections 2..6 -> I.2..I.6; Section 7 -> II.
% Subsections inherit as I.1, I.6.1, ...
\makeatletter
\renewcommand{\thesection}{%
  \ifnum\value{section}<7\relax
    ...
    \fi
  \else
    \@Roman{\numexpr\value{section}-5\relax}% 7->II, 8->III, ...
  \fi
}
...
Điều khiển như vậy được gọi là \textbf{bang-bang}: mỗi thành phần $u_i^*(t)$ chỉ nhận các giá trị $+1$ hoặc $-1$ (gần như mọi $t$), và chỉ \emph{chuyển} giữa $+1$ và $-1$ tại các \textbf{thời điểm chuyển mạch} (switching times) khi biểu thức quyết định đổi dấu. Mỗi lần điều khiển tối ưu thời gian chuyển từ trạng thái -1 lên 1 hoặc từ 1 xuống -1 thì được gọi là một lần bang - bang (nổ).
...
\section{Ứng dụng của Nguyên lý Cực đại}
Xét một bài toán điều khiển tối ưu với phương trình trạng thái là:
\begin{equation}
\dot{x}(t)=
\begin{bmatrix}
0 & 1\\
...
\noindent\textbf{Hệ quả:} Bằng cách áp dụng Nguyên lý Cực đại, hàm điều khiển tối ưu có dạng \emph{bang-bang}. Hàm chuyển mạch trong trường hợp này là một hàm tuyến tính theo thời gian, điều khiển tối ưu thời gian có dạng:
\begin{equation}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.82\textwidth]{hinh5.jpg}
% The image already contains the title; hide LaTeX caption text but keep numbering.
\captionsetup{labelformat=empty}
\caption{}
\label{fig:5}
\end{figure}

Hình 5 minh hoạ cách xây dựng quỹ đạo tối ưu từ một điểm xuất phát $(x_0,y_0)$. Quỹ đạo tối ưu (đường đậm) được ghép từ hai cung ứng với hai giá trị điều khiển biên; trong hình minh hoạ, cung cuối đi về gốc ứng với $u=+1$, còn cung trước đó ứng với $u=-1$ (đường nét đứt). Hai nhánh đường cong nét đứt $x_2=-\sqrt{2x_1}$ (với $x_1>0$) và $x_2=\sqrt{-2x_1}$ (với $x_1<0$) biểu diễn đường cong chuyển mạch, nơi chiến lược bang-bang đổi dấu.
